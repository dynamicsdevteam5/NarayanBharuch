...
{
    public class LifeSkillsProjectController : SessionController
    {
        private readonly LifeSkillsProjectRepository _repo = new LifeSkillsProjectRepository();
        // GET: Admin/LifeSkillsProject
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    if (data != null)
                    {
                        ...
                        var photos = Helper.FileHelper.GetFilesFromFolder("LifeSkillsProject", Convert.ToInt32(id));
                        if (photos.Count > 0)
                        {
        ...
        #region LifeSkillsProject Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(LifeSkillsProjectModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                if (id > 0)
                {
                    ViewBag.SuccessMessage = "LifeSkillsProject saved successfully. :)";
                    Helper.FileHelper.SaveToMainFolder("LifeSkillsProject", model.Photos, id);
                }
                else
                {
        ...
        #region Delete LifeSkillsProject
        ...
        public JsonResult FnPaging()
        {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/LifeSkillsProject/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {                
                string path = Server.MapPath("~/Photos/LifeSkillsProject/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/LifeSkillsProject/Temp/" + file.FileName + ",";
                }
