...
{
    public class StudentSubjectSeminarController : SessionController
    {
        // GET: Admin/StudentSubjectSeminar
        public ActionResult Index(int? id)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    using (StudentSubjectSeminarRepository _repo = new StudentSubjectSeminarRepository())
                    {
                        var data = _repo.GetById(Convert.ToInt32(id));
                        if (data != null)
                        {
                            ViewBag.DepartmentData = new SelectList(departments, "Id", "Name", data.DepartmentId);
                            var photos = Helper.FileHelper.GetFilesFromFolder("StudentSubjectSeminar", Convert.ToInt32(id));
                            if (photos.Count > 0)
                            {
        ...
        #region StudentSubjectSeminar Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(StudentSubjectSeminarModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                using (StudentSubjectSeminarRepository _repo = new StudentSubjectSeminarRepository())
                {
                    ...
                    if (id > 0)
                    {
                        ViewBag.SuccessMessage = "StudentSubjectSeminar saved successfully. :)";
                        Helper.FileHelper.SaveToMainFolder("StudentSubjectSeminar", model.Photos, id);
                    }
                    else
                    {
        ...
        #region Delete StudentSubjectSeminar
        ...
        public JsonResult FnPaging()
        {
            ...
            List<StudentSubjectSeminarModel> data = new List<StudentSubjectSeminarModel>();
            using (StudentSubjectSeminarRepository _repo = new StudentSubjectSeminarRepository())
            {
                data = _repo.GetList(iDisplayStart, iDisplayLength, searchpara, sortablecol, sortorder).ToList();
            }
            var count = "0";
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/StudentSubjectSeminar/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {
                string path = Server.MapPath("~/Photos/StudentSubjectSeminar/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/StudentSubjectSeminar/Temp/" + file.FileName + ",";
                }
