...
{
    public class InnovativExperimentController : SessionController
    {
        private readonly InnovativExperimentRepository _repo = new InnovativExperimentRepository();
        // GET: Admin/InnovativExperiment
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    if (data != null)
                    {
                        ...
                        var photos = Helper.FileHelper.GetFilesFromFolder("InnovativExperiment", Convert.ToInt32(id));
                        if(photos.Count > 0)
                        {
        ...
        #region InnovativExperiment Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(InnovativExperimentModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                if (id > 0)
                {
                    ViewBag.SuccessMessage = "InnovativExperiment saved successfully. :)";
                    Helper.FileHelper.SaveToMainFolder("InnovativExperiment", model.Photos, id);
                }   
                else
                {
        ...
        #region Delete InnovativExperiment
        ...
        public JsonResult FnPaging()
        {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/InnovativExperiment/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {                
                string path = Server.MapPath("~/Photos/InnovativExperiment/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/InnovativExperiment/Temp/" + file.FileName + ",";
                }
