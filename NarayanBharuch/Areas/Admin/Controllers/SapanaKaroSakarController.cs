...
{
    public class SapanaKaroSakarController : SessionController
    {
        // GET: Admin/SapanaKaroSakar
        public ActionResult Index(int? id)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    using (SapanaKaroSakarRepository _repo = new SapanaKaroSakarRepository())
                    {
                        var data = _repo.GetById(Convert.ToInt32(id));
                        if (data != null)
                        {
                            ViewBag.DepartmentData = new SelectList(departments, "Id", "Name", data.DepartmentId);
                            var photos = Helper.FileHelper.GetFilesFromFolder("SapanaKaroSakar", Convert.ToInt32(id));
                            if (photos.Count > 0)
                            {
        ...
        #region SapanaKaroSakar Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(SapanaKaroSakarModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                using (SapanaKaroSakarRepository _repo = new SapanaKaroSakarRepository())
                {
                    ...
                    if (id > 0)
                    {
                        ViewBag.SuccessMessage = "SapanaKaroSakar saved successfully. :)";
                        Helper.FileHelper.SaveToMainFolder("SapanaKaroSakar", model.Photos, id);
                    }
                    else
                    {
        ...
        public JsonResult FnPaging()
        {
            ...
            int sortCol = sortablecol != null ? Convert.ToInt32(sortablecol) : 0;
            List<SapanaKaroSakarModel> data = new List<SapanaKaroSakarModel>();
            using (SapanaKaroSakarRepository _repo = new SapanaKaroSakarRepository())
            {
                data = _repo.GetList(iDisplayStart, iDisplayLength, searchpara, sortCol, sortorder).ToList();
            }
            var count = "0";
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/SapanaKaroSakar/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {
                string path = Server.MapPath("~/Photos/SapanaKaroSakar/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/SapanaKaroSakar/Temp/" + file.FileName + ",";
                }
