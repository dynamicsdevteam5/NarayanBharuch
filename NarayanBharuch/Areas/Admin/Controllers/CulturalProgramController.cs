...
{
    public class CulturalProgramController : SessionController
    {
        // GET: Admin/CulturalProgram
        public ActionResult Index(int? id)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    using (CulturalProgramRepository _repo = new CulturalProgramRepository())
                    {
                        var data = _repo.GetById(Convert.ToInt32(id));
                        if (data != null)
                        {
                            ViewBag.DepartmentData = new SelectList(departments, "Id", "Name", data.DepartmentId);
                            var photos = Helper.FileHelper.GetFilesFromFolder("CulturalProgram", Convert.ToInt32(id));
                            if (photos.Count > 0)
                            {
        ...
        #region CulturalProgram Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(CulturalProgramModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                using (CulturalProgramRepository _repo = new CulturalProgramRepository())
                {
                    ...
                    if (id > 0)
                    {
                        ViewBag.SuccessMessage = "CulturalProgram saved successfully. :)";
                        Helper.FileHelper.SaveToMainFolder("CulturalProgram", model.Photos, id);
                    }
                    else
                    {
        ...
        public JsonResult FnPaging()
        {
            ...
            List<CulturalProgramModel> data = new List<CulturalProgramModel>();
            using (CulturalProgramRepository _repo = new CulturalProgramRepository())
            {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/CulturalProgram/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {
                string path = Server.MapPath("~/Photos/CulturalProgram/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/CulturalProgram/Temp/" + file.FileName + ",";
                }
