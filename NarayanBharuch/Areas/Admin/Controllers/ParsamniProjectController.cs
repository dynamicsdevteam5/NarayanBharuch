...
{
    public class ParsamniProjectController : SessionController
    {
        private readonly ParsamniProjectRepository _repo = new ParsamniProjectRepository();
        // GET: Admin/ParsamniProject
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                else
                {
                    ...
                    if (data != null)
                    {
                        ...
                        var photos = Helper.FileHelper.GetFilesFromFolder("ParsamniProject", Convert.ToInt32(id));
                        if (photos.Count > 0)
                        {
        ...
        #region ParsamniProject Post Data
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Index(ParsamniProjectModel model)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserId"])))
            {
                ...
                if (id > 0)
                {
                    ViewBag.SuccessMessage = "ParsamniProject saved successfully. :)";
                    Helper.FileHelper.SaveToMainFolder("ParsamniProject", model.Photos, id);
                }
                else
                {
        ...
        public JsonResult FnPaging()
        {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                subaray[4] = "";
                subaray[4] += "<a class='btn ink-reaction btn-floating-action btn-sm btn-primary fa fa-edit' data-ajax-complete='showEdit();scrolltop();' data-ajax='true' data-ajax-method='GET' data-ajax-mode='replace' data-ajax-update='#P_Main' href='/Admin/ParsamniProject/Index?id=" + data[i].Id + "' title='Edit'></a>";
        ...
        public JsonResult SaveToTemp()
        {
            ...
            try
            {   
                string path = Server.MapPath("~/Photos/ParsamniProject/Temp/");
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    imagefiles += "/Photos/ParsamniProject/Temp/" + file.FileName + ",";
                }
